...
#### 入力支援機能（v0.2.5で完成）
- **現在地プレースホルダー表示**
  - **インタラクティブモード**: プロンプトに現在地を表示（例：「パラグラフ番号 (Enter: 15)」）
  - **空入力時の自動入力**: Enterのみで現在地の番号が自動入力される
  - **パラグラフ追加時の特別処理**: 未定義パラグラフの場合のみデフォルト表示
- **InputHelperクラスによる統一実装**
  - 現在地の取得と表示の一元管理
  - 操作別の条件分岐（追加時は未定義のみ、他は常に表示）
  - 空入力処理の共通化
- **対象操作**
  - パラグラフ追加時：未定義パラグラフのみデフォルト表示
  - 選択肢追加時：現在地をEnterで自動入力
  - 選択肢選択時：現在地をEnterで自動入力
  - 直接移動時：現在地をEnterで自動入力
- **実装範囲と制約**
  - **インタラクティブモード**: 完全実装済み
  - **CLIモード**: 技術的制約により未実装
  - **Tab補完**: PTermの制約により実装見送り
...
- **現在地プレースホルダー表示** - 実装完了
  - インタラクティブモード：プロンプトに現在地を表示（例：「パラグラフ番号 (Enter: 15)」）
  - 空入力時のEnterで現在地の自動入力機能
  - パラグラフ追加時は未定義パラグラフの場合のみデフォルト表示
- **InputHelperクラスの実装**
  - GetDefaultText(): 現在地の取得
  - GetDefaultTextForAdd(): 追加時の特別処理（未定義パラグラフのみ）
  - ProcessEmptyInput(): 空入力時の自動入力処理
  - GetCLIPrompt(): CLIモード用プロンプト生成（未実装）
- **対象操作の実装**
  - パラグラフ追加時：未定義パラグラフのみデフォルト表示
  - 選択肢追加時：現在地をEnterで自動入力
  - 選択肢選択時：現在地をEnterで自動入力
  - 直接移動時：現在地をEnterで自動入力
- **実装範囲**
  - インタラクティブモードのみ対応（使用頻度が高いため）
  - CLIモードは技術的制約により未対応
  - Tab補完機能は実装見送り（PTermの制約）
