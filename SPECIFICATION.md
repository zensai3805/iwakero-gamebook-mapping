...
#### 入力支援機能（v0.2.5で実装中）
- **現在地プレースホルダー表示**
  - **インタラクティブモード**: パラグラフ番号入力時に現在地を半透明で表示（WithDefaultText）
  - **CLIモード**: プロンプトに現在地を表示（例：`パラグラフ番号 [現在地: 15]: `）
  - 両モードで現在地の視覚的ヒント提供
- **最小限のTab補完**
  - **インタラクティブモード**: PTermによるTab補完機能（現在地を最優先候補）
  - **CLIモード**: 標準入力の制約によりTab補完は技術的に困難
  - **両モード共通**: 空入力時のEnterで現在地の自動入力
  - ゲームブック特性を考慮した実用的な補完機能
- **操作別Tab補完候補**
  - パラグラフ追加時：現在地のみ（新規パラグラフ番号入力）
  - 選択肢追加時：現在地のみ（対象パラグラフ番号入力）
  - 選択肢選択時：現在地のみ（対象パラグラフ番号入力）
  - 直接移動時：現在地のみ（移動先パラグラフ番号入力）
  - **原則**：A to B のパラグラフ入力では、Aは一律で現在地を候補とする
- **対象操作の限定**
  - パラグラフ追加・選択肢追加・選択肢選択・直接移動時の番号入力
  - 選択肢の遷移先は対象外（新規パラグラフが前提のため）
- **技術的制約**
  - **CLIモード**: 標準入力の制約により半透明表示・Tab補完は不可
  - **インタラクティブモード**: PTermの機能により全機能実装可能
  - **実装優先順位**: インタラクティブモード優先（使用頻度が高いため）
...
### v0.2.5 - 入力支援機能実装 🔄 **実装中**
- **現在地プレースホルダー表示** - WithDefaultText機能
  - パラグラフ番号入力時に現在地を半透明で表示
  - 「パラグラフ番号: 15」形式での視覚的ヒント
  - インタラクティブモード・CLIモード両対応
- **最小限のTab補完機能**
  - 現在地を最優先候補として表示
  - 空入力時のEnterで現在地の自動入力
  - 複雑な候補生成は除外（ゲームブック特性に配慮）
- **対象範囲の限定**
  - パラグラフ追加時の番号入力
  - 選択肢追加時のパラグラフ番号入力
  - 選択肢選択時のパラグラフ番号入力
